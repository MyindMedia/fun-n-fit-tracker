            ...
            <div className="space-y-4">
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                    ...
                    <div className="flex items-center gap-4 mb-5 pb-4 border-b border-slate-100">
                        <div className="w-14 h-14 rounded-full bg-gradient-to-br from-brand-blue to-blue-400 flex items-center justify-center text-white font-black text-2xl shadow-md">
                            {parent.fullName.charAt(0).toUpperCase()}
                        </div>
                        <div className="min-w-0">
                            <h2 className="font-black text-slate-900 text-lg leading-tight">{parent.fullName}</h2>
                    ...
                    <div className="space-y-2 mb-5">
                        {parent.linkedStudents.length === 0 && (
                            <p className="text-slate-400 text-sm text-center py-4 bg-slate-50 rounded-xl">No athletes linked yet</p>
                        )}
                            ...
                            return (
                                <div key={s.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                                    ...
                                    <div className="flex-grow min-w-0">
                                        ...
                                        <div className="text-[10px] text-slate-400">{house.mascot} {house.name} · {s.points} pts</div>
                                    </div>
                                    <button onClick={() => handleUnlink(parent.id, s.id)}
                                        className="text-red-400 hover:text-red-600 text-xs font-bold px-2 py-1 rounded-lg hover:bg-red-50 transition-colors flex-shrink-0">
                                        Remove
                                    </button>
                                </div>
                            );
                    ...
                    <div className="flex gap-2">
                        <select value={linkStudentId} onChange={e => setLinkStudentId(e.target.value)}
                            className="flex-grow px-3 py-2 rounded-xl border-2 border-slate-100 bg-slate-50 text-sm font-bold text-slate-900 focus:border-brand-blue outline-none">
                        ...
                        <button onClick={() => handleLinkStudent(parent.id)} disabled={!linkStudentId || linking}
                            className="px-4 py-2 bg-brand-blue text-white rounded-xl font-black text-sm disabled:opacity-40 hover:opacity-90 transition-opacity">
                            {linking ? '...' : 'Link'}
                        </button>
                    </div>
                    {linkMsg && (
                        <p className={`text-sm font-medium mt-2 ${linkMsg.startsWith('✅') ? 'text-green-600' : 'text-red-500'}`}>{linkMsg}</p>
                    )}
                </div>
            </div>
        );
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="font-black text-slate-900 text-base">👨‍👩‍👧‍👦 Parent Accounts</h2>
                    <p className="text-slate-400 text-xs">{parents.length} registered</p>
                </div>
                <button onClick={() => setActiveView('create')}
                    className="px-4 py-2 bg-brand-blue text-white rounded-xl font-black text-sm hover:opacity-90 transition-opacity shadow-sm">
                    + New Parent
                </button>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8 text-center">
                    ...
                    <h3 className="font-black text-slate-900 mb-2">No Parent Accounts Yet</h3>
                    <p className="text-slate-400 text-sm mb-4">Create the first parent account to get started.</p>
                    <button onClick={() => setActiveView('create')}
                        className="px-5 py-2.5 bg-brand-blue text-white rounded-xl font-black text-sm hover:opacity-90">
                ...
                <div className="space-y-2">
                    {parents.map(parent => (
                        <button key={parent.id} onClick={() => { setSelectedParent(parent); setLinkMsg(null); setLinkStudentId(''); }}
                            className="w-full bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center gap-4 hover:border-blue-200 hover:bg-blue-50/30 transition-all text-left active:scale-[0.99]">
                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-brand-blue to-blue-400 flex items-center justify-center text-white font-black text-base flex-shrink-0 shadow-sm">
                                {parent.fullName.charAt(0).toUpperCase()}
                            </div>
                            <div className="flex-grow min-w-0">
                                <div className="font-black text-slate-900 text-sm truncate">{parent.fullName}</div>
                                <div className="text-[11px] text-slate-400 truncate">{parent.email}</div>
                                {parent.phone !== '—' && <div className="text-[10px] text-slate-400">📱 {parent.phone}</div>}
                            </div>
                            <div className="flex-shrink-0 flex flex-col items-end gap-1">
                                ...
                                <div className="flex -space-x-1.5">
                                    {parent.linkedStudents.slice(0, 3).map(s => (
                                        <img key={s.id}
                                            src={s.avatarUrl || `https://api.dicebear.com/7.x/adventurer/svg?seed=${s.id}`}
                                            className="w-6 h-6 rounded-full border-2 border-white object-cover" alt="" />
                                    ))}
                                </div>
                            </div>
...
const OnboardingWizard: React.FC<{ onDone: () => void; onCancel: () => void }> = ({ onDone, onCancel }) => {
    ...
    return (
        <div className="max-w-md mx-auto">
            {/* Cancel */}
            <button onClick={onCancel} className="flex items-center gap-2 text-sm font-black text-slate-500 hover:text-slate-800 transition-colors mb-4">
                ← Back to Parents
            </button>

            {/* Branded header */}
            <div className="bg-gradient-to-br from-brand-blue via-blue-600 to-indigo-700 rounded-2xl p-6 text-white text-center mb-4 shadow-lg relative overflow-hidden">
                {/* Decorative blobs */}
                ...
                <div className="relative">
                    ...
                    <h1 className="text-xl font-black uppercase tracking-tight">Fun 'N Fit Academy</h1>
                    <p className="text-blue-100 text-sm mt-1">Parent Onboarding</p>
                </div>
            </div>
                ...
                {STEPS.map((s, i) => (
                    <React.Fragment key={s.step}>
                        <div className={`flex items-center gap-1.5 ${step >= s.step ? 'opacity-100' : 'opacity-30'} transition-opacity`}>
                            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black transition-all ${step > s.step ? 'bg-green-500 text-white' : step === s.step ? 'bg-brand-blue text-white shadow-md' : 'bg-slate-200 text-slate-500'
                                }`}>
                                {step > s.step ? '✓' : s.step}
                            </div>
                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-wide hidden sm:block">{s.label}</span>
                        </div>
                        {i < STEPS.length - 1 && (
                            <div className="flex-grow h-0.5 bg-slate-200 rounded-full overflow-hidden">
                                <div className={`h-full bg-brand-blue rounded-full transition-all duration-500 ${step > s.step ? 'w-full' : 'w-0'}`} />
                            </div>
                        )}
                    </React.Fragment>
                ))}
            ...
            {step === 1 && (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4">
                    <div>
                        <h2 className="font-black text-slate-900 text-base">👋 Let's start with the basics</h2>
                        <p className="text-slate-400 text-sm">Tell us about the parent joining the Fun 'N Fit family.</p>
                    </div>
            ...
            {step === 2 && (
                <form onSubmit={handleSubmitAccount} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4">
                    <div>
                        <h2 className="font-black text-slate-900 text-base">🔐 Create their account</h2>
                        <p className="text-slate-400 text-sm">Set up login credentials for <strong>{form.fullName}</strong>.</p>
                    </div>
                    {error && (
            ...
            {step === 3 && (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8 text-center space-y-4">
                    ...
                    <div>
                        <h2 className="font-black text-slate-900 text-xl">Welcome to the team, {createdName}!</h2>
