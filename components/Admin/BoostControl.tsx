...
// positive award academy-wide; spends and refunds stay 1:1. The Double XP
// event control below it multiplies ONLY the Volt XP mirror, never points.
const BoostControl: React.FC = () => {
  const [mult, setMult] = useState<number>(1);
  const [xpMult, setXpMult] = useState<number>(1);
  const [busy, setBusy] = useState(false);

  useEffect(() => gameCenter.subscribePointMultiplier(setMult), []);
  useEffect(() => gameCenter.subscribeXpMultiplier(setXpMult), []);
  ...
  const applyXpBoost = async (value: number) => {
    setBusy(true);
    try {
      await gameCenter.setXpMultiplier(value);
      window.dispatchEvent(new CustomEvent('coach-toast', {
        detail: { message: value > 1 ? `${value}x XP event is ON` : 'XP event turned off' },
      }));
    } catch (err: any) {
      alert(err?.message || 'Failed to update the XP event');
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 mt-4 mb-3">
        <span style={{ color: xpActive ? 'var(--pz-volt)' : 'var(--pz-text)' }}><Ic.Chart size={22} /></span>
        <div className="flex-grow">
          <div className="font-black text-white uppercase tracking-wide text-[15px]">Double XP Event</div>
          <div className="text-xs" style={{ color: xpActive ? 'var(--pz-volt)' : 'var(--pz-text)' }}>
            {xpActive ? `${xpMult}x XP active — Volt Levels climb faster` : 'Off — XP matches points earned'}
          </div>
        </div>
      </div>
      <div className="grid grid-cols-4 gap-2">
        {BOOST_OPTIONS.map(b => {
          const isActive = xpMult === b;
          return (
            <button
              key={`xp-${b}`}
              onClick={() => applyXpBoost(b)}
              disabled={busy}
              className="touch-btn min-h-[44px] py-2 text-sm font-black transition-all disabled:opacity-50"
              style={{
                clipPath: NOTCH_SM,
