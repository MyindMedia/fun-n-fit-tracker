  ...
  return (
    <div className="pz-scope fixed inset-0 z-[200] bg-black/70 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in">
      <div className="pz-card w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        <div className="p-8 flex justify-between items-center shrink-0" style={{ background: 'var(--pz-panel-2)', borderBottom: '1px solid var(--pz-border)' }}>
          <div>
        ...
        <div className="flex-grow overflow-y-auto p-8 custom-scrollbar space-y-4">
          {rosterStudents.map(s => (
            <div key={s.id} className="pz-card-sm p-6 flex flex-col md:flex-row justify-between items-center gap-6 group hover:border-[#CBFE1C]/40 transition-all" style={{ background: 'var(--pz-panel-2)' }}>
              <div className="flex items-center gap-6 flex-grow">
                <img src={s.avatarUrl} className="w-16 h-16 rounded-full bg-white/10 border-2 border-white/10 object-cover" />
                <div>
                  <div className="text-xl font-black text-white">{s.fullName}</div>
                 ...
                 <div className="flex gap-2 items-center">
                    {[1,3,5,10].map(val => (
                      <button
                        ...
                        className="bg-white/10 border border-white/10 text-white w-12 h-10 font-black active:scale-95 transition-all flex items-center justify-center"
                      >
                        +{val}
                      </button>
                    ))}
                    <input type="number" className="w-16 h-10 border border-white/10 bg-[#171C27] text-white placeholder-white/30 text-center font-black focus:border-[#CBFE1C] outline-none" placeholder="Custom" onKeyDown={(e) => { const v = Number((e.target as HTMLInputElement).value); if (e.key==='Enter' && v>0) supabaseService.addPoints(s.id, v, 'MANUAL', `${game.title} Custom`, adminName); }} />
                 </div>
              </div>
            </div>
          ))}
        ...
        <div className="p-8 flex justify-end gap-4 shrink-0" style={{ background: 'var(--pz-panel-2)', borderTop: '1px solid var(--pz-border)' }}>
